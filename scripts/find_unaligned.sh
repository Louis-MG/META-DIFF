...
It output the following files :
	- unaligned.fasta file containing the unaligned sequences.
"
}
...
# je pourrais remplacer cette etape par faire directemetn dans awk le calule
#cut -f1 "$m8" | sort -n | uniq > "$output"/tmp.txt
