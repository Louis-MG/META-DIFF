...
class Train:
    ...
    def init_neptune(self, h_params_dict):
        try:
            ...
            print(
                f"[WARNING] Neptune run could not be initialized: {e}. Disabling Neptune logging for this run."
            )
    ...
    def train(self, h_params):
        ...
        scores = {
            group: {
                ...
                "f1": [],
                "cluster_metrics": {
                    f"{cluster_id}_acc": [] for cluster_id in np.unique(clusters)
                },
            }
        ...
        while h < self.n_splits:
            ...
            train_inds = [
                x
                for x in train_nums
                if x not in np.concatenate((valid_inds, test_inds))
            ]
            ...
            ys = {
                group: self.data["y"].iloc[inds].copy()
            ...
            preds = {
                group: classifier.predict(Xs[group].values)
                for group in ["train", "valid", "test"]
            }
            try:
                probas_preds = {
                    group: np.max(classifier.predict_proba(Xs[group].values), 1)
                    for group in ["train", "valid", "test"]
                }
                probas_true = {
                    group: classifier.predict_proba(Xs[group].values)
                    for group in ["train", "valid", "test"]
                }
                for group in ["train", "valid", "test"]:
                    probas_true[group] = [
                        x[y] for x, y in zip(probas_true[group], ys[group])
                    ]
            except:
                probas_preds = preds
                probas_true = preds
            ...
            for group in ["train", "valid", "test"]:
                scores[group]["mcc"] += [
                    metrics.matthews_corrcoef(ys[group], preds[group])
                ]
                scores[group]["acc"] += [accuracy_score(ys[group], preds[group])]
                scores[group]["ari"] += [
                    metrics.adjusted_rand_score(ys[group], cs[group])
                ]
                scores[group]["ami"] += [
                    metrics.adjusted_mutual_info_score(ys[group], preds[group])
                ]
                scores[group]["precision"] += [
                    metrics.precision_score(
                        ys[group], preds[group], average="binary", zero_division=0
                    )
                ]
                scores[group]["TPR"] += [
                    metrics.recall_score(ys[group], preds[group], average="binary")
                ]
                scores[group]["f1"] += [
                    metrics.f1_score(ys[group], preds[group], average="binary")
                ]
                scores[group]["TNR"] += [
                    metrics.recall_score(
                        ys[group], preds[group], pos_label=0, average="binary"
                    )
                ]
                scores[group]["FNR"] += [
                    1 - metrics.recall_score(ys[group], preds[group], average="binary")
                ]
                scores[group]["FPR"] += [
                    1
                    - metrics.recall_score(
                ...
                scores[group]["informedness"] += [
                    scores[group]["TPR"][-1] + scores[group]["TNR"][-1] - 1
                ]
                scores[group]["kappa"] += [
                    metrics.cohen_kappa_score(ys[group], preds[group])
                ]
                    ...
                    try:
                        if x_dict[group].iloc[cluster_mask].shape[0] == 1:
                            y_pred = classifier.predict(
                                x_dict[group].iloc[cluster_mask].values.reshape(1, -1)
                            )
                        else:
                            y_pred = classifier.predict(
                                x_dict[group].iloc[cluster_mask].values
                            )
                    except:
                    ...
                    for metric in scores[group].keys():
                        if metric != "cluster_metrics":
                            ...
                            for cluster_metric in scores[group][metric].keys():
                                try:
                                    run[f"{group}/{cluster_metric}"].log(
                                        scores[group][metric][cluster_metric][-1]
                                    )
                                except:
                                    pass
        ...
        self.pbar.update(1)
        self.log_stuff(
            run, h_params_dict, ys_dict, preds_dict, probas_dict_true, scores
        )
        ...
        # Print groups mcc
        print(
            f'Current mccs: train: {np.mean(scores["train"]["mcc"])},'
    ...
    def log_stuff(
        self, run, h_params_dict, ys_dict, preds_dict, probas_dict_true, scores
    ):
        ...
        for group in ["train", "valid", "test"]:
            if "cluster_metrics" in scores[group].keys():
                for cluster_metric in scores[group]["cluster_metrics"].keys():
                    scores[group][f"cluster_{cluster_metric}"] = scores[group][
                        "cluster_metrics"
                    ][cluster_metric]
        ...
        scores = {
            group: {k: np.mean(v) for k, v in scores[group].items()}
            for group in scores.keys()
        }
        pd.DataFrame(scores).to_json(
        ...
        for group in ["train", "valid", "test"]:
            ...
            plt.title(
                f"{group} mcc: {np.round(scores[group]['mcc'], 3)}, acc: {np.round(scores[group]['acc'], 3)}"
            )
