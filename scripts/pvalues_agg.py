...
def getArgs():
    parser = argparse.ArgumentParser(
        description="Aggregates pvalues from kmdiff to a single p-value per unitig."
    )
    parser.add_argument(
        "-k",
        "--kmdiff",
        type=str,
        dest="kmdiff_input_path",
        action="store",
        required=True,
...
def verif_output(path: Union[str, bytes, os.PathLike], prefix: str):
    ...
    if os.path.isfile(path + "/" + prefix + ".aggregated.fa"):
        print(f"ERROR: folder {path} contains a '{prefix}.aggregated.fa' file already.")
        sys.exit()


def write_output(
    path: Union[str, bytes, os.PathLike], list_unitigs: list[object], prefix: str
):
    """
    Writes output.
    :param path : string or path-like object to the output directory.
        ...
        for line in f:
            if line.startswith(">"):
                ...
                try:
                    ...
                    size += sum(map(sys.getsizeof, kmer_2_pvalue.values())) + sum(
                        map(sys.getsizeof, kmer_2_pvalue.keys())
                    )
                    print(size)
                    exit()
...
def load_unitigs(
    path: Union[str, bytes, os.PathLike], kmer_dict: dict[str, float]
) -> list[object]:
    """
    ...
    with open(path) as f:
        for line in f:
            if line.startswith(">"):
                ...
                sequence = line
                unitig = make_unitig(
                    sequence=sequence, kmer_dict=kmer_dict, header=header
                )
...
def main(
    kmdiff_input_path: Union[str, bytes, os.PathLike],
    unitigs_input_path: Union[str, bytes, os.PathLike],
